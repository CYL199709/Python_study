...
def callbackFunc(e, x, y, f, p):
    if e == cv2.EVENT_LBUTTONDOWN:
        print(points_3d[y][x])
        if abs(points_3d[y][x][2]) < 5000:   #原本是3000
            print("当前距离:" + str(abs(points_3d[y][x][2])))
        else:
            print("当前距离过大或请点击色块的位置")

if __name__ == '__main__':
    # 读取数据集的图片
    iml = cv2.imread('picture_l.jpg')  # 左图
    imr = cv2.imread('picture_r.jpg')  # 右图
    ...
    #生成深度图
    displ,dispr= disparity_SGBM(iml_rectified, imr_rectified)  # 这里传入的是未经立体校正的图像，因为我们使用的middleburry图片已经是校正过的了
    disp_l = np.divide(displ.astype(np.float32), 16.)  # 除以16得到真实视差（因为SGBM算法得到的视差是×16的）
    disp_r = np.divide(displ.astype(np.float32), 16.)
    cv2.imwrite('shichal.jpg', disp_l)
    cv2.imwrite('shichar.jpg', disp_r)
    cv2.imshow('shichal.jpg', disp_l)
    cv2.imshow('shichar.jpg',disp_r)

    # 计算像素点的3D坐标（左相机坐标系下）
    points_3d = cv2.reprojectImageTo3D(disp_l, Q)  # 可以使用上文的stereo_config.py给出的参数

    #点击测距
    cv2.setMouseCallback("shichal.jpg", callbackFunc, None)
