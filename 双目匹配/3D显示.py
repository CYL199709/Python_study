#author yuqiang time:2020/12/8
#此程序实现三维可视化点云
#检验图片为1.png，1.pgm，1.ply
...
class point_cloud_generator():
    ...
    def __init__(self, rgb_file, depth_file, pc_file, focal_length, scalingfactor):  #焦距、比例因子
        ...
        self.depth = Image.open(depth_file).convert('I')   #32位灰度图像 0-255 注意
    ...
    def calculate(self):
        t1 = time.time()
        depth = np.asarray(self.depth).T              # 其实是0-255
    ...
    def show_point_cloud(self):
        pcd = o3d.io.read_point_cloud(self.pc_file)
            # read_point_cloud(self.pc_file)
        o3d.visualization.draw_geometries([pcd])
        # draw_geometries([pcd])


a = point_cloud_generator('picture_l.jpg', 'shichal.jpg', '2.ply',
                          focal_length=5.25558800e+02, scalingfactor=100)
a.calculate()
a.write_ply()
a.show_point_cloud()
df = a.df
